...
現状で設計したプロセッサは,5ステージのマルチサイクル方式のハーバード方式のプロセッサである.
...
プログラムカウンタ
: 実行中の命令のアドレスを保持する16bitレジスタ.

汎用レジスタ
: 16bitのレジスタを8つ.

条件コードレジスタ
: 演算命令結果を保持するレジスタ. 条件分岐命令のときに用いる.
...
| ニモニック | 説明       | 動作                                   | ra  | rb                                                 |
|:-----------|:-----------|:---------------------------------------|:----|:---------------------------------------------------|
| LI         | 即値ロード | `r[rb] = signext(d)`                   | 000 | rb                                                 |
...
![フェーズフローチャート](./images/phase_flow.png){#fig:phase_flow}

フェーズフローチャートを,[@fig:phase_flow]に示す.

## データフローチャート

データフローチャートは,[@fig:flow_p1], [@fig:flow_p2],[@fig:flow_p3],[@fig:flow_p4], [@fig:flow_p5] に示される通りである.
